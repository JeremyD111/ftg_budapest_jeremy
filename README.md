# Controlador Reactivo F1TENTH - Algoritmo Follow the Gap (FTG)
...
git clone https://github.com/JeremyD111/ftg_budapest_jeremy.git
```
...
### Paso 3: Ejecutar el simulador F1Tenth
...
### Paso 4: Ejecutar el nodo controlador Follow The Gap
...
cd ~/F1Tenth-Repository
source install/setup.bash
ros2 run ftg_budapest_jeremy ftg_controller
```
---

## 2. Enfoque Utilizado: Follow the Gap (FTG)

El método Follow the Gap (FTG) es un controlador reactivo basado directamente en la información del sensor LIDAR. Su objetivo es encontrar el espacio libre más amplio frente al vehículo y orientar el volante hacia el centro de ese espacio, garantizando navegación segura y estable en entornos con obstáculos.

El funcionamiento de FTG se puede resumir en tres etapas principales:

### Procesamiento del LIDAR

El arreglo de distancias del LIDAR es filtrado para eliminar lecturas ruidosas o demasiado cercanas, aplicando un umbral mínimo de seguridad. Esto permite descartar obstáculos que no deben ser considerados como parte de un “hueco libre”.

### Identificación del Gap Más Amplio

El algoritmo analiza la secuencia filtrada y detecta segmentos continuos donde las lecturas son mayores al umbral definido. Cada segmento representa un posible camino libre (gap).
El controlador selecciona el gap de mayor longitud angular, pues es el que más espacio ofrece para navegar sin colisiones.

### Selección del Ángulo y Control del Vehículo

Una vez encontrado el gap óptimo, el vehículo apunta hacia el centro geométrico del mismo.
El ángulo de dirección final se calcula en función del índice central del gap y, opcionalmente, se ajusta según la distancia máxima detectada.
La velocidad también puede adaptarse: normalmente es mayor cuando el vehículo tiene un camino libre amplio y menor cuando el entorno es estrecho o incierto.

---
## 3. Estructura del Còdigo

El controlador está organizado en módulos que separan claramente la lectura de sensores, el procesamiento del LIDAR, la lógica del algoritmo FTG y el sistema de conteo de vueltas. A continuación se describe brevemente cada bloque principal del código:

### Inicializacion del odo y Suscripciones

El constructor `__init__()` crea el nodo ROS2 reactive_follow_gap, inicializa suscripciones a:

- `/scan` → lecturas del LIDAR
- `/ego_racecar/odom` → odometría para el conteo de vueltas
- Publicadores `/drive` y `/lap_info → comandos de movimiento y mensajes informativos

También define todos los parámetros del controlador FTG (radio del bubble, suavizado, steering gain…) y los parámetros del cronómetro.
...
- Convierte los rangos a un array NumPy.
- Sustituye ceros y distancias mayores al rango útil.
- Suaviza las lecturas con un filtro promedio móvil.

Esto reduce ruido y hace más estable la selección del gap.
...
### Detección del Gap Más Grande

La función `find_largest_gap()`:

- Recorre el arreglo de “espacio libre” (después del bubble).
- Identifica todos los segmentos continuos con valores > 0.
- Devuelve el gap más grande según su longitud angular.
...
### Seleccion del Punto Objetivo

`select_best_point()` elige el índice dentro del gap con la mayor distancia del LIDAR, que tiende a ser el centro más despejado hacia donde dirigir el vehículo.
...
`compute_adaptive_speed()` ajusta la velocidad según la distancia del objetivo:

- Distancias pequeñas → velocidad baja
- Distancias amplias → velocidad alta

Esto mejora estabilidad en curvas y agresividad en rectas.
...
### Callback Principal del LIDAR (FTG)

lidar_callback() implementa todo el algoritmo FTG:

1. Suaviza LIDAR
2. Identifica el obstáculo más cercano
3. Aplica bubble
4. Encuentra el mayor gap
5. Selecciona el mejor punto
6. Convierte ese punto en ángulo de dirección
7. Ajusta la velocidad con el modelo adaptativo
8. Publica el comando /drive
...
- Detecta entradas/salidas de la zona de meta.
- Calcula y registra tiempos de vuelta.
- Identifica el mejor tiempo.
- Finaliza la competencia después de 10 vueltas.
...
`main()` inicializa ROS2, ejecuta el nodo en modo spin y realiza el apagado limpio
